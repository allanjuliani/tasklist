...
class ApiTest(APITestCase):
    ...
    def test_list_add(self):
        ...
        self.assertEqual(
            response.json(), {'message': 'New list created', 'data': {'list_id': 2}}
        )
    ...
    def test_list_full_update(self):
        ...
        self.assertEqual(
            response.json(),
            {
                'detail': _(f'List {list_id} updated'),
                'data': {
                    'id': self.list_base.id,
                    'name': data.get('name'),
    ...
    def test_list_delete(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(
            response.json(), {'detail': f'List {self.list_base.id} deleted'}
        )
    ...
    def test_tag_add(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(
            response.json(), {'message': 'New Tag created', 'data': {'tag_id': 2}}
        )
    ...
    def test_task_load(self):
        ...
        self.assertEqual(
            response.json(),
            {
                ...
                'remind_me_on': self.task_base.remind_me_on.strftime(
                    '%Y-%m-%d %H:%M:%S'
                ),
                ...
                'created': self.task_base.created.astimezone().strftime(
                    '%Y-%m-%d %H:%M:%S'
                ),
                'updated': self.task_base.created.astimezone().strftime(
                    '%Y-%m-%d %H:%M:%S'
                ),
                'tags': [
                    {
    ...
    def test_task_full_update(self):
        ...
        self.assertEqual(
            response.json(),
            {
                'detail': _(f'Task {task_id} updated'),
                'data': {
                    'id': self.task_base.id,
    ...
    def test_task_delete(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(
            response.json(), {'detail': f'Task {self.task_base.id} deleted'}
        )
